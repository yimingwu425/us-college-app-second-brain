...
v4.0-modern 是一次重新设计：它不再围绕阶段、固定话术和多层记忆关卡运行，而是把现代 AI 当作有判断力的申请伙伴。维护者可以在 `src/runtime/` 中迭代少量高密度模块，普通用户只需要复制 `dist/CLAUDE.md` 或 `dist/AGENTS.md`，再配合 `templates/vault/` 就能开始使用。
...
1. 下载或 clone 本仓库。
2. 按你的工具选择运行文件：Claude 用 `dist/CLAUDE.md`，Codex/通用 agent 用 `dist/AGENTS.md`。
